...
>
> This README highlights the most prominent ones. Some work together as a system; others are standalone solutions.

*Built a suite of real-time, event-driven monitoring tools in MQL5 to automate decision workflows, eliminate manual intervention, and enforce operational discipline — the same problems DevOps solves, just in a different domain.*

*The domain is financial markets. The engineering mindset is DevOps:*

- **Real-time monitoring** — live market data observation with state awareness
- **Event detection & processing** — trigger-based logic responding to condition changes
- **Alerting pipelines with noise control** — signal filtering to reduce false positives
- **Data aggregation & state tracking** — multi-source inputs consolidated into actionable output
- **Operational guardrails** — automated checks that enforce rules and prevent bad decisions
...
*This project highlights four custom indicators designed as real-time monitoring and alerting tools for trading environments.*
...
*All indicators follow an event-driven architecture.*

*Each component reacts to live market data, processes events, maintains state, and triggers actions only when conditions are met.*
...
Event Processing Logic
      ↓
State Tracking (in-memory/persistence)
      ↓
Alert / UI Update
```
...
- **Implemented stateful tracking**\
for real-time aggregation and context-aware decisions

- **Built noise-controlled alerting systems**\
using thresholds and one-time triggers

- **Developed data preprocessing tools**\
to clean, filter, and structure raw inputs before processing

- **Applied parsing and normalization logic**\
to handle unstructured external data
